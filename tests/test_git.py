...
def test_open_repo_current_dir(temp_repo, tmp_path: Path, monkeypatch):
...
def test_open_repo_with_path(temp_repo, tmp_path: Path):
    """Test opening repo with explicit path."""
    repo = git.open_repo(tmp_path)
    assert repo is not None


def test_get_current_branch(repo_with_feature):
    """Test getting current branch name."""
    branch = git.get_current_branch(repo_with_feature)
    assert branch == "feature"


def test_get_current_branch_detached_head(temp_repo):
...
def test_get_default_branch_from_origin_head(temp_repo):
...
def test_get_default_branch_fallback_main(temp_repo):
    """Test fallback to main when origin/HEAD not set."""
    default = git.get_default_branch(temp_repo)
    assert default == "main"


def test_get_default_branch_fallback_master(tmp_path: Path):
...
def test_get_default_branch_none(tmp_path: Path):
